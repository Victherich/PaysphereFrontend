...
const TransactionAlert = () => {
  const [transactions, setTransactions] = useState([]);
  const [newTransactions, setNewTransactions] = useState([]); // State to track only new transactions
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const userToken = useSelector(state => state.userToken);
        ...
        newTransactions.map(transaction => (
          <TransactionCard key={transaction._id}>
            <TransactionInfo>
              ...
              <P>
                <Strong>Amount:</Strong> NGN {transaction.amountPaid}
              </P>
              <P>
              ...
              <P>
                <Strong>Status:</Strong> {transaction.message}
              </P>
            </TransactionInfo>
          </TransactionCard>
        ))
      ) : (
        !loading && <NoTransactionMessage>No new transactions yet.</NoTransactionMessage>
      )}
    </TransactionContainer>
  );
};
